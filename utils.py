    ...
    def __init__(
        ...
        for method in self.methods:
            if method == "original_sequences":
                # ✅ Aquí corregimos la ruta real de tus videos originales
                method_root = self.root / "original_sequences" / "youtube" / self.compression / "videos"
            else:
                method_root = self.root / "manipulated_sequences" / method / self.compression
        
            if method_root.exists():
                self.video_paths.extend(list(method_root.rglob("*.mp4")))
            else:
                log.warning(f"No se encontró la carpeta para {method} en {method_root}")
